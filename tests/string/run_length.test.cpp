...
void test_run_length() {
    ...
    assert(res1.size() == 3);
    assert(res1[0] == make_pair('a', 3));
    assert(res1[1] == make_pair('b', 2));
    assert(res1[2] == make_pair('c', 1));
    // ケース2: 全て同じ文字
    string s2 = "aaaaa";
    vector<pair<char, int>> res2 = run_length(s2);
    assert(res2.size() == 1);
    assert(res2[0] == make_pair('a', 5));
    // ケース3: 交互に現れる文字
    // "abab" -> [('a', 1), ('b', 1), ('a', 1), ('b', 1)]
    string s3 = "abab";
    vector<pair<char, int>> res3 = run_length(s3);
    assert(res3.size() == 4);
    assert(res3[2].first == 'a' && res3[2].second == 1);
    // ケース4: 空文字列
    string s4 = "";
    ...
    assert(res5.size() == 1);
    assert(res5[0].first == 'z' && res5[0].second == 1);
}
/**
 * @brief ランレングス圧縮のテスト
 */
