...
data:
  ...
  attributes:
    document_title: "\u661F\u7A7A\u6728 \u533A\u9593\u52A0\u7B97 \u533A\u9593min/max\u53D6\
      \u5F97"
    links:
  ...
  bundledCode: "#line 2 \"library/segtree/starry_sky_tree.hpp\"\n/**\n * @brief \u661F\
    \u7A7A\u6728 \u533A\u9593\u52A0\u7B97 \u533A\u9593min/max\u53D6\u5F97\n * @note\
    \ https://rsk0315.hatenablog.com/entry/2019/06/21/174953\n *\n * `rev = true`\
    \ \u3067\u533A\u9593min\u306B\u5909\u66F4\n */\ntemplate <bool rev = false> struct\
    \ StarrySkyTree {\n  private:\n    int N, sz, rv, log = 1;\n    vector<int> node;\n\
    \    void init() {\n        while ((1ll << log) < N) ++log;\n        node.assign((sz\
    \ = 1ll << log) << 1, -INF);\n        rv = (rev ? -1 : 1);\n    }\n    int _star(int\
  ...
  code: "#pragma once\n/**\n * @brief \u661F\u7A7A\u6728 \u533A\u9593\u52A0\u7B97\
    ...
    \ *\n * `rev = true` \u3067\u533A\u9593min\u306B\u5909\u66F4\n */\ntemplate <bool\
    \ rev = false> struct StarrySkyTree {\n  private:\n    int N, sz, rv, log = 1;\n\
    \    vector<int> node;\n    void init() {\n        while ((1ll << log) < N) ++log;\n\
    \        node.assign((sz = 1ll << log) << 1, -INF);\n        rv = (rev ? -1 :\
    \ 1);\n    }\n    int _star(int i) {\n        int mx = max(node[i << 1 | 0], node[i\
    \ << 1 | 1]);\n        node[i << 1 | 0] -= mx;\n        node[i << 1 | 1] -= mx;\n\
    \        return mx;\n    }\n    void star(int i) {\n        node[i] += _star(i);\n\
    \    }\n    int sum(int i) {\n        int ans = node[i];\n        while (i >>=\
    \ 1) ans += node[i];\n        return ans;\n    }\n\n  public:\n    StarrySkyTree(int\
    ...
    \    }\n    int operator[](int i) {\n        return sum(i + sz) * rv;\n    }\n\
    \    vector<int> getall() {\n        vector<int> res(N);\n        for (int i =\
    \ 0; i < N; ++i) res[i] = sum(i + sz) * rv;\n        return res;\n    }\n    void\
    \ set(int p, const int &x) {\n        apply(p, p + 1, x - sum(p + sz) * rv);\n\
    \    }\n    void act(int p, const int &x) {\n        apply(p, p + 1, x);\n   \
    \ }\n    int get(int l, int r) {\n        int ans = -INF;\n        for (l += sz,\
    \ r += sz; l < r; l >>= 1, r >>= 1) {\n            if (l & 1) ans = max(ans, sum(l++));\n\
    \            if (r & 1) ans = max(ans, sum(--r));\n        }\n        return ans\
...
layout: document
redirect_from:
- /library/library/segtree/starry_sky_tree.hpp
- /library/library/segtree/starry_sky_tree.hpp.html
title: "\u661F\u7A7A\u6728 \u533A\u9593\u52A0\u7B97 \u533A\u9593min/max\u53D6\u5F97"
---
