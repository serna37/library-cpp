...
data:
  ...
  bundledCode: "#line 2 \"library/polynomial/fps/formal_power_series.hpp\"\n#include\
    ...
    \ <typename T> struct FormalPowerSeries : vector<T> {\n    using vector<T>::vector;\n\
    ...
    \ = rfact[i] * T(i);\n        P p(*this);\n        for (int i = 0; i < n; i++)\
    \ p[i] *= fact[i];\n        p = p.rev();\n        P bs(n, T(1));\n        for\
    \ (int i = 1; i < n; i++)\n            bs[i] = bs[i - 1] * c * rfact[i] * fact[i\
    \ - 1];\n        p = (p * bs).pre(n);\n        p = p.rev();\n        for (int\
    \ i = 0; i < n; i++) p[i] *= rfact[i];\n        return p;\n    }\n};\ntemplate\
    \ <typename Mint> using FPS = FormalPowerSeries<Mint>;\n#line 3 \"library/polynomial/fps/bernoulli_number.hpp\"\
